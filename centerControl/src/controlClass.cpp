...
#pragma region 参观控制
	...
	else if (msg > VISIT_PAGE_BEGIN && msg < VISIT_PAGE_END)
	{
		...
		{
			if (msg >= VISIT_CONTROL_PAGE_1_LIGHT_ALL_ON && msg <= VISIT_CONTROL_PAGE_1_LIGHT_LOOP)
			{
				vector<string> ips = videoMgr.getIPs(0);
				for (int i = 0;i < ips.size(); i++)
				{
					if (msg == VISIT_CONTROL_PAGE_1_LIGHT_ALL_ON)
					{
						sendUdpCmd(ips[i], 19999, "seque_light_all_on");
					}
					else if (msg == VISIT_CONTROL_PAGE_1_LIGHT_ALL_OFF)
					{
						sendUdpCmd(ips[i], 19999, "seque_light_all_off");
					}
					else if (msg == VISIT_CONTROL_PAGE_1_LIGHT_LOOP)
					{
						sendUdpCmd(ips[i], 19999, "seque_light_all_loop");
					}
					
				}
			}
			else if (msg >= VISIT_CONTROL_PAGE_1_DAI_JI && msg <= VISIT_CONTROL_PAGE_1_XIN_XI_LIU)
			{
				vector<string> ips = videoMgr.getIPs(1);
				for (int i = 0; i < ips.size(); i++)
				{
					if (msg == VISIT_CONTROL_PAGE_1_DAI_JI)
					{
						sendUdpCmd(ips[i], 8888, "1");
					}
					else if (msg == VISIT_CONTROL_PAGE_1_XIN_XI_LIU)
					{
						sendUdpCmd(ips[i], 8888, "2");
					}

				}
			}
			else if(msg > VISIT_CONTROL_PAGE_1_XIN_XI_LIU && msg < VISIT_CONTROL_PAGE_1_END)
			{
				int index = 2;
				int temp = msg - VISIT_CONTROL_PAGE_1_XIN_XI_LIU - 1;
